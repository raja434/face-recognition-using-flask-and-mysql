...
def error_handle(error_message, status=500, mimetype='application/json'):
    return Response(json.dumps({"error": {"message": error_message,"info":"failed","info_id":"2,4,6"}}), status=status, mimetype=mimetype)


def get_user_by_id(user_id):
    user = {}
    results = app.db.select('SELECT users.id, users.name, users.created, faces.id, faces.user_id, faces.filename,faces.created FROM users LEFT JOIN faces ON faces.user_id = users.id WHERE users.id = %s',
        [user_id])
    index = 0
    for row in results:
        # print(row)
...
def homepage():
    output = json.dumps({"api": '0.1',"developed by":"rajasekhar mutukuri" })
    ...
    if 'file' not in request.files:
        ...
        if file.mimetype not in app.config['file_allowed']:
            ...
            try:
                ...
                if user_id:
                    user = get_user_by_id(user_id)
                    message = {"message": "Hey we found {0} ".format(user["name"]),
                            "user": user,"Info":"sucess","Info_id":"1"}
                ...
                message = {"message": "No face in the image","Info_id":"3"}
                return success_handle(json.dumps(message))
